...
impl<'a> State<'a> {
  fn init(&mut self) -> Result<()> {
    ...
    */
    ...
    if self.git_dir.read_dir()?.count() == 0 {
      self.create_command_git_clone(GIT_URL).arg(".").spawn()?.wait()?;
    }
    Ok(())
  }
  ...
  fn perf_commit(&mut self, hash: &str) -> Result<Vec<Stats>> {
    ...
    results.iter_mut().for_each(|x| x.hash = Some(hash.to_owned()));
    Ok(results)
  }
  fn perf_file(&mut self, file: &Path) -> Result<Vec<Stats>> {
    ...
    results.iter_mut().for_each(|x| x.file = Some(file.to_string_lossy().into_owned()));
    Ok(results)
  }

  /// Run a function after compiling a HVMC file into a binary
  fn with_compiled<T>(&mut self, file: &Path, f: impl FnOnce(&mut Self, &Path) -> Result<T>) -> Result<T> {
  ...
  fn parse_output(&mut self, s: &str) -> Result<Stats> {
    Ok(Stats {
      time: self.re_time.captures(s).map(|x| x.extract::<1>().1[0].to_owned()),
      rwps: self.re_rwps.captures(s).map(|x| x.extract::<1>().1[0].to_owned()),
      rwts: self.re_rwts.captures(s).map(|x| x.extract::<1>().1[0].to_owned()),
      ..Default::default()
    })
  }
  ...
  fn create_command_cargo_run(&mut self) -> Command {
    ...
    command.current_dir(&self.crate_dir).arg("run").arg("--release").arg("--");
    command
  }
  fn is_git_ancestor(&mut self, ancestor: &str, descendant: &str) -> Result<bool> {
    let mut c = self.create_command_git();
    c.arg("merge-base").arg("--is-ancestor").arg(ancestor).arg(descendant);
